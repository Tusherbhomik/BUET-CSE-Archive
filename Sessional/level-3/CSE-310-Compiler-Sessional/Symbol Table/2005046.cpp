...
// Splits line by spaces into at most maxTokens slots.
// Clears all slots first so stale values from previous calls don't linger.
void splitLine(string &line, string *arr, int maxTokens)
{
    for (int i = 0; i < maxTokens; i++) arr[i] = "";
    ...
    for (ull i = 0; i < line.length(); ++i)
    {
        if (line[i] == delimiter)
        {
            if (arrIndex < maxTokens)
                arr[arrIndex++] = line.substr(startIndex, i - startIndex);
            startIndex = i + 1;
        }
    }

    if (startIndex < line.length() && arrIndex < maxTokens)
...
int main()
{
    ...
    string words[MAX_TOKENS];

    while (getline(cin, str))
    {
        splitLine(str, words, MAX_TOKENS);
