...
// Helper function to check if ray is obscured (for shadow calculation)
...
double Sphere::intersect(Ray* r, double* color, int level) {
    ...
    // Calculate lighting from point lights - FIXED: Use size_t for comparison
    for (size_t i = 0; i < pointLights.size(); i++) {
        ...
        if (!isObscured(intersectionPoint + normal * 1e-6, pointLights[i].light_pos)) {
            ...
            double phongValue = std::max(0.0, reflected.dot(viewDir));
    ...
    // Calculate lighting from spot lights - FIXED: Use size_t for comparison
    for (size_t i = 0; i < spotLights.size(); i++) {
        ...
        // Check if point is within spotlight cone - FIXED: Use unary minus operator
    ...
    if (level < recursion_level) {
        ...
        // Find nearest intersecting object - FIXED: Use size_t for comparison
        for (size_t i = 0; i < objects.size(); i++) {
            double temp_color[3];
...
double GeneralQuadric::intersect(Ray* r, double* color, int level) {
    ...
    // Check which intersection point is valid and within bounding box
    if (t1 > 1e-6) {
...
double Floor::intersect(Ray* r, double* color, int level) {
    ...
    if (abs(r->dir.z) < 1e-6) {
        return -1; // Ray is parallel to floor
    }
