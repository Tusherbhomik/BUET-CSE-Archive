...
public:
    ...
    // MODIFIED: getColorAt method to support texture
    Vector3D getColorAt(Vector3D point) override {
        if (useTexture && textureData) {
            // Convert world coordinates to texture coordinates
            double u = (point.x - reference_point.x) / floorWidth;
            double v = (point.y - reference_point.y) / floorWidth;
            
            // Handle texture tiling/wrapping
            u = u - floor(u); // Keep fractional part for tiling
            v = v - floor(v);
