...
#!/usr/bin/env python3
"""
Compare Toyoda and Modified Toyoda algorithms and generate visualizations.
...
output_dir = Path(_fig_base) / "toyoda_comparison"
output_dir.mkdir(exist_ok=True, parents=True)

print("="*80)
print("TOYODA vs MODIFIED TOYODA COMPARISON ANALYSIS")
print("="*80)

# Calculate improvement percentage
df['profit_improvement'] = ((df['improved_toyoda_profit'] - df['toyoda_profit']) / df['toyoda_profit'] * 100)

better_count = (df['improved_toyoda_profit'] > df['toyoda_profit']).sum()
worse_count = (df['improved_toyoda_profit'] < df['toyoda_profit']).sum()
equal_count = (df['improved_toyoda_profit'] == df['toyoda_profit']).sum()
...
print(f"\nRuntime:")
print(f"  Toyoda Mean: {df['toyoda_ms'].mean():.2f} ms")
print(f"  Improved Mean: {df['improved_toyoda_ms'].mean():.2f} ms")
print(f"  Ratio: {(df['improved_toyoda_ms'].mean() / df['toyoda_ms'].mean()):.2f}x")
...
plt.title('Modified Toyoda vs Toyoda: Profit Improvement by Dataset', fontsize=15, fontweight='bold')
...
print("✓ Saved: improvement_by_dataset_bar.png")
plt.close()
...
plt.title('Modified Toyoda vs Toyoda: Average Improvement by Dataset Group',
...
bars1 = plt.bar(x - width/2, group_runtime['toyoda_ms'], width,
                label='Toyoda', alpha=0.8, color='#3498db', edgecolor='black', linewidth=1.5)
bars2 = plt.bar(x + width/2, group_runtime['improved_toyoda_ms'], width,
                label='Modified Toyoda', alpha=0.8, color='#e74c3c', edgecolor='black', linewidth=1.5)

plt.xlabel('Dataset Group', fontsize=14, fontweight='bold')
plt.ylabel('Average Execution Time (ms)', fontsize=14, fontweight='bold')
plt.title('Runtime Comparison: Toyoda vs Modified Toyoda', fontsize=16, fontweight='bold', pad=20)
