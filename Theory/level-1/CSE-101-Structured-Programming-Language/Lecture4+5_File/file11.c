...
void saveToFile(Contact c[], int N) {
    int i = 0;
...
void saveToFileBinary(Contact c[], int N) {
    int i = 0;
    fp = fopen("contacts", "wb");
    for (i = 0; i < N; i++) {
        fwrite(&c[i], sizeof(Contact), 1, fp);
    }
    // better way
    //fwrite(c, sizeof(Contact), N, fp); 
    fclose(fp);
}

int loadFromFile(Contact c[]) {
    int i = 0;
...
int loadFromFileBinary(Contact c[]) {
    int i = 0;
    fp = fopen("contacts", "rb");
    if (fp == NULL) return 0;
    while(fread(&c[i], sizeof(Contact), 1, fp) == 1) {
        i++;
    }
    fclose(fp);
    return i;
...
int main()
{
    ...
    //i = loadFromFile(phoneBook);
    i = loadFromFileBinary(phoneBook);
    while(1) {
        ...
        if (choice == 1) {
            ...
            //saveToFile(phoneBook, i);
            saveToFileBinary(phoneBook, i);
            exit(0);
        }
